...
// sorts object keys for stable output, except everywhere below "imports" and
// "exports": their condition objects match first-wins, so template order is
// semantic and passes through verbatim
function canonicalize(value: unknown, preserveOrder: boolean): unknown {
  if (Array.isArray(value)) {
    return value.map((item) => canonicalize(item, preserveOrder));
  }
  ...
  for (const key of keys) {
    result[key] = canonicalize(value[key], preserveOrder || key === "imports" || key === "exports");
  }
  return result;
}
...
class PackageJsonGenerator {
  ...
  private resolveExtends(entry: string, baseDirectory: string): string {
    ...
    console.error(`Error: template '${entry}' not found on template path: ${this.searchPath.join(":")}`);
    process.exit(1);
  }
}

function main(): void {
  const generator = new PackageJsonGenerator(process.env["DERAMMO_NPM_PACKAGE_TEMPLATE_PATH"] ?? "");
